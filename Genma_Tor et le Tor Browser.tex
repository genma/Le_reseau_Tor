...
\justifying{
Tor est l'acronyme de The Onion Router, littéralement « le routeur oignon », en référence au \emph{routage en oignon}.
\begin{itemize}
...
Concrêtement, utiliser Tor peut permettre :
\begin{itemize}
\justifying{
...
\\
$\Rightarrow$ Il faut utiliser une connexion httpS (les clés étant négociées entre le client et le serveur web final).
}
\end{frame}
...
\\~\\
Après sélection des différents "noeuds-relais" dans le réseau, le client effectue un échange de clefs de sessions (Diffie-Hellman).  Il y a une première clé de chiffrement pour le nœud d'entrée, une second clé pour le nœud du milieu et une dernière pour le nœud de sortie.
}
\begin{itemize}
\justifying{
...
\item Comme avec un navigateur "normal", ne consultez pas/n'ouvrez pas de documents téléchargé pendant que vous êtes connecté si ceux-ci présentent "un risque". 
...
\begin{itemize}
\justifying{
\item Il n'y a pas de plugin flash (mais les vidéos HTML5 passent).
